...
function renderContent(content: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i]
    if (line.startsWith('## ')) {
      elements.push(<h2 key={key++} className="text-2xl font-bold text-gray-900 mt-10 mb-4">{line.replace('## ', '')}</h2>)
    } else if (line.startsWith('**') && line.endsWith('**')) {
      elements.push(<p key={key++} className="font-semibold text-gray-900 mb-2">{line.replace(/\*\*/g, '')}</p>)
    } else if (line.match(/^\d+\.\s/)) {
      elements.push(<li key={key++} className="ml-6 mb-2 text-gray-700 list-decimal">{line.replace(/^\d+\.\s/, '').replace(/\*\*(.*?)\*\*/g, '$1')}</li>)
    } else if (line.startsWith('- ')) {
      elements.push(<li key={key++} className="ml-6 mb-2 text-gray-700 list-disc">{line.replace('- ', '').replace(/\*\*(.*?)\*\*/g, '$1')}</li>)
      ...
      elements.push(<p key={key++} className="text-gray-700 leading-relaxed mb-4" dangerouslySetInnerHTML={{ __html: html }} />)
    }
  }
  ...
  for (const el of elements) {
    if ((el as React.ReactElement).type === 'tr') {
      ...
      if (tableRows.length > 0) {
        wrapped.push(<div key={key++} className="overflow-x-auto my-6"><table className="w-full border border-gray-200 rounded-lg overflow-hidden"><tbody>{tableRows}</tbody></table></div>)
        tableRows = []
      }
      wrapped.push(el)
    }
...
export default function BlogPostContent({ post, slug }: { post: BlogPost | null; slug: string }) {
  ...
  useEffect(() => {
    ...
    const allPosts = [...customPosts, ...BLOG_POSTS.filter(p => !builtInSlugs.has(p.slug))]
      ...
      <section className="bg-gradient-to-br from-blue-900 via-blue-800 to-blue-900 pt-32 pb-16">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3 mb-4">
            <span className={`text-xs font-semibold px-3 py-1 rounded-full ${categoryColors[activePost.category] ?? 'bg-white/20 text-white'}`}>
              {activePost.category}
            </span>
            <span className="text-blue-200 text-sm">{activePost.date}</span>
            <span className="text-blue-200 text-sm">· {activePost.readTime}</span>
          </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        {activePost.coverImage && (
          <div className="mb-12 rounded-2xl overflow-hidden shadow-lg">
            ...
            <img src={activePost.coverImage} alt={activePost.title} className="w-full object-cover max-h-96" />
          </div>
        )}
        ...
        <div className="mt-16 bg-gradient-to-r from-blue-600 to-blue-800 rounded-2xl p-8 text-center text-white">
          ...
          <p className="text-blue-100 mb-6">Get a free quote from Spider Logistics. Fast, reliable, and transparent.</p>
          <Link href="/contact" className="inline-block bg-white text-blue-700 font-semibold px-8 py-3 rounded-full hover:bg-blue-50 transition-colors">
            Contact Us
          </Link>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {related.map((r) => (
                <Link key={r.slug} href={`/blog/${r.slug}`} className="group block border border-gray-100 rounded-xl p-6 hover:shadow-md transition-shadow">
                  ...
                  <h3 className="mt-3 font-bold text-gray-900 group-hover:text-blue-600 transition-colors">{r.title}</h3>
